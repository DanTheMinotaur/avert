...
plugin.checkReadableStreams = (obj) => {

    function isReadableStream(o) {
        const isin = o instanceof stream.Readable
        console.log({ isin })
        // if (isin) {
        //     console.log({ obj })
        // }
    ...
    if (isReadableStream(obj)) {
        readableStreams._readableStreams = obj;
        return readableStreams;
    }

    for (const prop in obj) {
        if (obj.hasOwnProperty(prop) && isReadableStream(obj[prop])) {
            readableStreams._readableStreams[prop] = obj[prop];
        }
    }

    const streams = Object.keys(readableStreams._readableStreams)

    if (streams.length) {
        streams.forEach(function(prop) {
            delete obj[prop];
        });
    }

    readableStreams._toSanitize = obj;
    return readableStreams;
};
...
plugin.avert = (obj, options, firstPass, secondPass) => {
    ...
    if (plugin.checkReadableStreams(cleansed)) return cleansed;

    // const fileStreams = plugin.checkReadableStreams(obj);
    //
    // // console.log({ obj })
    //
    // console.log({ fileStreams })
    // console.log(Object.keys(fileStreams))
    //
    // cleansed = fileStreams._toSanitize;
    //
    ...
    // if (fileStreams._readableStreams) {
    //     cleansed = {
    //         ...cleansed,
    //         ...fileStreams._readableStreams
    //     }
    // }
